...
class Shell(object):
    ...
    def ssh(self, instance_data):
        ...
        ssh_basic = "ssh -o StrictHostKeyChecking=no -i %s %s@%s -p %s " % (keyfile, user, host, port)
    ...
    def converge(self, instance_data):
        ...
        rsync_cmd = "rsync -avze 'ssh -o StrictHostKeyChecking=no -i %s' %s %s@%s:%s" % (keyfile, self.copy_from, user, host, self.copy_to)
